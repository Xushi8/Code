...
uint64_t mult32to64(uint32_t x, uint32_t y)
{
    return __emulu(x, y);
}
#else
...
{
template <size_t N>
vec_t<N> loadu(const vec_t<N>* input)
{
...
template <size_t N>
vec_t<N> xorv(vec_t<N> a, vec_t<N> b)
{
...
template <size_t N>
vec_t<N> mul(vec_t<N> a, vec_t<N> b)
{
...
template <size_t N>
vec_t<N> add(vec_t<N> a, vec_t<N> b)
{
...
template <size_t N>
uint_t<N> avalanche(uint_t<N> hash)
{
    if constexpr (N == 32)
    {
        hash ^= hash >> 15;
...
template <size_t N>
uint_t<N> round(uint_t<N> seed, uint_t<N> input)
{
...
uint64_t mergeRound64(hash64_t acc, uint64_t val)
{
    val = round<64>(0, val);
    acc ^= val;
...
void endian_align_sub_mergeround(hash64_t& hash_ret, uint64_t v1, uint64_t v2, uint64_t v3, uint64_t v4)
{
...
hash_t<64> rrmxmx(hash_t<64> h64, uint64_t len)
{
...
void combine_16(void* dest, hash128_t h128)
{
    writeLE<64>(dest, readLE<64>(dest) ^ h128.low64);
    writeLE<64>((uint8_t*)dest + 8, readLE<64>((uint8_t*)dest + 8) ^ h128.high64);
}

void accumulate_512(void* XXH_RESTRICT acc, const void* XXH_RESTRICT input, const void* XXH_RESTRICT secret)
{
...
void accumulate(uint64_t* XXH_RESTRICT acc, const uint8_t* XXH_RESTRICT input, const uint8_t* XXH_RESTRICT secret, size_t nbStripes)
{
...
void hash_long_internal_loop(uint64_t* XXH_RESTRICT acc, const uint8_t* XXH_RESTRICT input, size_t len, const uint8_t* XXH_RESTRICT secret, size_t secretSize)
{
...
template <size_t N>
hash_t<N> len_1to3(const uint8_t* input, size_t len, const uint8_t* secret, uint64_t seed)
{
    if constexpr (N == 64)
    {
...
template <size_t N>
hash_t<N> len_4to8(const uint8_t* input, size_t len, const uint8_t* secret, uint64_t seed)
{
...
template <size_t N>
hash_t<N> len_9to16(const uint8_t* input, size_t len, const uint8_t* secret, uint64_t seed)
{
    if constexpr (N == 64)
    {
...
template <size_t N>
hash_t<N> len_0to16(const uint8_t* input, size_t len, const uint8_t* secret, uint64_t seed)
{
    if (XXH_likely(len > 8))
    {
...
uint64_t mix_16b(const uint8_t* XXH_RESTRICT input, const uint8_t* XXH_RESTRICT secret, uint64_t seed)
{
    uint64_t const input_lo = readLE<64>(input);
...
uint128_t mix_32b(uint128_t acc, const uint8_t* input1, const uint8_t* input2, const uint8_t* secret, uint64_t seed)
{
...
template <size_t N>
hash_t<N> len_17to128(const uint8_t* XXH_RESTRICT input, size_t len, const uint8_t* XXH_RESTRICT secret, uint64_t seed)
{
    if constexpr (N == 64)
    {
...
template <size_t N>
hash_t<N> len_129to240(const uint8_t* XXH_RESTRICT input, size_t len, const uint8_t* XXH_RESTRICT secret, uint64_t seed)
{
    if constexpr (N == 64)
    {
...
template <size_t N>
hash_t<N> xxhash3_impl(const void* XXH_RESTRICT input, size_t len, hash64_t seed, const void* XXH_RESTRICT secret = default_secret, size_t secretSize = secret_default_size)
{
...
void generate_secret(void* secret_buffer, size_t secret_size, const void* custom_seed, size_t seed_size)
{
    if (seed_size == 0)
    {
...
int main()
{
    ...
    struct cmp
    {
        bool operator()(xxh::hash_t<128> lhs, xxh::hash_t<128> rhs)
        {
            return lhs < rhs;
        }
    };
    ...
    int n;
    cin >> n;
    set<xxh::hash_t<64>> st;
    set<xxh::hash_t<128>, cmp> st1;
    string s;
    for (int i = 0; i < n; i++)
    {
        cin >> s;
        // xxh::hash_t<64> ret = xxh::xxhash3<64>(s);
        // st.emplace(ret);
        auto ret1 = xxh::xxhash3<128>(s);
        st1.emplace(ret1);
    }
